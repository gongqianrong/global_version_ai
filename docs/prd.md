...
Rakutao International 是一款面向全球用户的日本商品聚合代购移动应用。平台聚合 Mercari、Rakuma、TOBU、SuperDelivery 等多个日本电商平台的商品数据，以 Rakutao 自有品牌统一呈现，隐藏底层来源平台。用户可通过搜索浏览、粘贴商品链接（指定购买）或代购直播三种方式下单，由 Rakutao 在日本代为购买，商品入库日本仓库后由用户申请打包并支付国际运费发货。首批重点市场为东南亚（越南等）和台湾，通过地区配置化架构快速扩展至全球更多市场。

**v2.0 新增能力**：
- **代购直播**：代购者在日本实体店/展会现场直播，用户实时下单
- **WMS 仓储系统**：独立仓储管理微服务，支持入库→打包→发货全流程
- **直接支付**：Apple Pay / Google Pay / PayPal 直接支付，无需预充值钱包
- **内容分级**：支持成人/擦边类商品，含年龄验证与地区合规控制
...
App 默认日语，支持繁体中文和英语，所有商品信息通过 LLM 实时翻译为用户所选语言。以日本法人实体上架应用商店，中国大陆地区不可用。所有地区站点共享统一 UI 框架和交互模式，仅在内容和视觉风格上做地区差异化，减少用户跨站学习成本。
...
| **钱包 → 钱包 + 直接支付** | v1 钱包预充值：解耦支付碎片化与消费场景，适配代购天然时间差（多次环节扣款合并为一次充值），资金沉淀创造粘性 | **v2 新增直接支付（Apple Pay / Google Pay / PayPal）**：降低新用户首单门槛——"还没买东西就先充钱"是心理障碍。直接支付允许零摩擦首单体验，高频用户仍可使用钱包提升效率。**两种支付方式并存而非替代** |
| **JPY 单一标价** | 将汇率风险转化为用户已知变量。平台避免汇率对赌，用户获得价格确定性 | 不变 |
| **日本法人 + 默认日语** | 法律合规（降低跨境电商审核复杂度）+ 信任构建（"日本公司卖日本商品"） | 不变 |
| **屏蔽中国大陆** | 战略聚焦——避免与淘宝/京东正面竞争，集中资源在东南亚蓝海市场 | 不变 |
...
| FR-ONB-002 | 语言选择页 | P0 | 首次启动展示语言选择页，提供三个选项：繁體中文、日本語（默认高亮）、English，选择后点击 "sign in" 进入登录页 |
...
| FR-AUTH-002 | 邮箱注册 | P0 | 点击 "Sign Up" 进入注册流程，填写邮箱、密码完成注册 |
...
| FR-AUTH-006 | 平台账号关联 | P1 | 登录成功后进入平台选择页，展示可关联的第三方平台（Mercari、乐酷淘等），用户可选择 Rakutao 独立账号或关联已有平台账号，点击"続ける"继续 |
...
| FR-PD-003 | 价格展示 | P0 | 显示商品价格（JPY 原价，粗体红色）和汇率换算参考价（灰色小字，如 "Exchange rate: 0.93..."） |
...
| FR-PD-007 | 运费信息 | P0 | 展示运费详情：日本国内运费（Within Japan）、服务费（Service Charge）、国际运费（Global Delivery: Fee calculated after weighing） |
...
| ID | 需求 | 优先级 | 描述 |
|----|------|--------|------|
| FR-CART-001 | 购物车商品列表 | P0 | 按卖家分组展示商品，每组显示：Seller ID、卖家名称、平台标签（如"楽可買"）；每条商品显示缩略图、名称、价格（JPY 红色粗体） |
...
| FR-OL-005 | 支持平台展示 | P1 | 页面底部展示支持的平台 Logo 列表：TOBU、minne、NICOL、netsea、SuperDelivery、BEAMS 等，让用户了解可代购的平台范围 |
...
| FR-WAL-002 | 充值功能 | P0 | 余额旁提供"チャージ"（Charge）按钮（红色），点击进入充值流程，支持 Apple Pay / Google Pay / PayPal / 本地支付方式 |
...
| FR-ORD-015 | 运费确认与支付 | P0 | 打包完成后系统推送实际运费，用户确认后通过钱包余额或直接支付（Apple Pay/Google Pay/PayPal）支付运费 |
...
| FR-PROF-004 | 收货地址管理 | P0 | "Shipping Address" 入口，点击进入收货地址列表，支持增删改查收货地址 |
| FR-PROF-005 | 修改密码 | P1 | "Change Password" 入口，点击进入密码修改流程 |
| FR-PROF-006 | 语言设置 | P0 | "Language Settings" 入口，点击可切换 App 语言（日/繁中/英），切换后 App 全局语言即时更新 |
...
| FR-CR-004 | 搜索过滤 | P0 | 商品搜索/列表默认过滤 M/A 级内容，仅已通过年龄验证的用户可见对应等级商品 |
| FR-CR-005 | 地区合规屏蔽 | P0 | 按各国/地区法规配置屏蔽特定内容等级（如部分国家需完全屏蔽 A 级），通过 RegionConfig 控制 |
| FR-CR-006 | 推送限制 | P1 | 推送/推荐不主动推送 A 级商品；M 级仅推送给已验证用户 |
...
| FR-WMS-004 | 用户支付运费 | P0 | 用户确认运费后通过钱包余额或直接支付（Apple Pay/Google Pay/PayPal）支付国际运费 |
...
| ID | 需求 | 优先级 | 描述 |
|----|------|--------|------|
| FR-PAY-001 | Apple Pay 支付 | P0 | iOS 用户支持通过 Apple Pay 直接支付商品款、运费、补差价，无需预充值钱包 |
| FR-PAY-002 | Google Pay 支付 | P0 | Android 用户支持通过 Google Pay 直接支付 |
| FR-PAY-003 | PayPal 支付 | P0 | 所有用户支持通过 PayPal 直接支付，覆盖全球主要市场 |
| FR-PAY-004 | 钱包余额支付 | P0 | 保留钱包余额支付方式，用户可选择钱包或直接支付 |
| FR-PAY-005 | 本地支付方式 | P1 | 按地区提供本地支付补充（如越南 MoMo/ZaloPay、韩国 KakaoPay 等），通过 RegionConfig 按需配置 |
| FR-PAY-006 | 支付方式选择 | P0 | 结算页展示可用支付方式列表（按地区和设备过滤），用户选择偏好支付方式 |
...
> 当采集的源平台（Mercari、Rakuma、TOBU 等）出现宕机或不可用时，通过服务端缓存和客户端离线缓存保障用户浏览和购买的连续性。
...
| NFR-006 | 地区限制 | 大陆不可用 | P0 | 中国大陆 IP 不可访问 App 服务；应用商店中国大陆区不可下载。其他地区面向全球开放 |
...
When   启动页加载完成后自动跳转
Then   显示语言选择页，包含繁體中文、日本語（默认高亮选中）、English 三个选项
  And  底部显示 "sign in" 按钮
...
Given  用户进入商品详情页
When   页面加载完成
Then   显示商品 JPY 原价（红色粗体，如 "JPY 23600"）
...
```
Given  用户在指定购买页面
When   粘贴一个有效的商品 URL（如 Mercari 商品链接）
Then   链接显示在输入框中
...
When   选择新的语言（如从 English 切换到繁體中文）
Then   App 全局 UI 文案立即切换为繁體中文
  And  语言偏好保存至本地和服务端
...
When   进入充值页面
Then   展示可用的支付方式（Apple Pay / Google Pay / PayPal / 本地支付方式）
...
When   用户打开运费确认页面
Then   显示打包明细（件数、实际重量、体积）和计算出的国际运费
  And  用户可选择支付方式：钱包余额 / Apple Pay / Google Pay / PayPal
  And  支付成功后订单状态从 Packed 变更为 Shipped（仓库发货后）

Given  用户钱包余额不足以支付运费
When   选择钱包支付
Then   提示余额不足，引导切换到直接支付或先充值
```
...
### FR-PAY-001/002/003: Apple Pay / Google Pay / PayPal 直接支付

```
Given  用户在结算页（商品款 / 运费 / 补差价）
When   选择 Apple Pay（iOS）/ Google Pay（Android）/ PayPal
Then   调起对应支付 SDK 完成支付
  And  无需预充值钱包余额
  And  支付成功后订单状态正常流转
  And  支付记录同步记入钱包流水明细

Given  用户设备不支持 Apple Pay（如 Android 设备）
When   进入结算页
Then   Apple Pay 选项不展示，仅展示该设备可用的支付方式
```
...
```
Given  Mercari 源站不可用，系统自动切换缓存模式
When   用户搜索 "handbag"
Then   搜索结果基于缓存数据返回
...
- 完整的注册/登录流程（邮箱 + 社交登录 + 条款同意）
- 启动引导（启动页 + 语言选择 + 平台关联）
- 首页商品浏览（列表展示、分类标签、搜索 + LLM 翻译、个性化推荐）
- 商品详情页（全部信息展示 + 加入购物车 + 立即购买 + 收藏 + 内容分级）
- 购物车（列表管理、价格变动提示、结算购买）
- 指定购买（链接输入 + 备注 + 提交 + 解锁平台实时搜索）
- **新购买流程**（下单→采购→入库→打包→支付运费→发货，10 态订单生命周期）
- **WMS 仓储系统**（入库、打包、运费计算、发货、高价值商品处理）
- **支付方式**（Apple Pay / Google Pay / PayPal 直接支付 + 钱包余额 + 本地支付）
- 钱包（余额展示、充值、订单列表 + 状态管理 + 运单追踪 + 流水记录）
- 收藏夹（收藏/取消、列表展示、关注卖家、批量编辑）
- 个人中心（个人信息、收货地址、语言设置、成人内容设置、工单系统）
- **内容分级**（G/M/A 三级 + 年龄验证 + 地区合规屏蔽）
...
| 术语 | 英文 | 日文 | 定义 |
|------|------|------|------|
| 源平台 | Source Platform | ソースプラットフォーム | 被聚合的日本电商平台（Mercari、Rakuma、TOBU 等） |
| 指定购买 | Order Link | 指定購入 | 用户粘贴商品链接由平台代购的购买方式 |
| 代购直播 | LiveStream | ライブ配信 | 代购者在日本现场直播，用户实时下单 |
| 钱包 | Wallet | ウォレット | 用户的 JPY 余额账户，支持充值和消费 |
| 直接支付 | Direct Payment | ダイレクト決済 | 通过 Apple Pay / Google Pay / PayPal 直接支付，无需预充值 |
